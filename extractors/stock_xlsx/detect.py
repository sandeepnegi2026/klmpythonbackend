def detect_excel_layout(rows):
    ...
    # r15_klm_venus_op_pur_sp_sale_ss_cr_db_adj_cstk_xls -- VENUS PHARMA (Ahmedabad) KLM
    # "Stock and Sales report" abbreviated-header .XLS (Item Name | Pack | <2 month-end
    # dates> | Op. | Pur | SP | Sale | SS | SVal | Cr. | Db. | Adj. | C Stk | C Val | Ord.).
    # The bare date columns + "Ord." fuzz-collide with the movement synonyms so `tabular`
    # mis-binds the movement columns and ~77% of rows fail sanity even though the source
    # reconciles 266/266. BHAGYODAY AGENCIES ships the byte-identical COLUMN header but a
    # SPARSE body (many zero-movement rows) that the positional parser drops (18 real rows
    # -> 6) while `tabular` reads it perfectly (18/18), so the gate ALSO requires the
    # "VENUS PHARMA" letterhead on the title rows (sheet row 0) -- the only stable token
    # separating the two exports. Any other vendor reusing this KLM export keeps falling
    # through to `tabular` (fail-safe = today's behaviour).
    # NOTE: r15_klm_venus_op_pur_sp_sale_ss_cr_db_adj_cstk_xls (VENUS PHARMA "KLM Statment
    # June_26.XLS") is NOT gated. A letterhead-keyed gate ("venuspharma" + the Op./Pur/SP/
    # Sale/SS + Cr./Db./Adj./C Stk header) also matched the GREEN historical VENUS baseline
    # "VENUS PHARMA (A.BAD) - DERMACOR.XLS" (Feb-26, Dec/Jan prev-month cols) and downgraded
    # it GREEN(tabular)->AMBER(partial) — the positional parser reconciles only on the June
    # combined multi-division book, not single-division historicals. The only separator is
    # the reporting month (Apr/May vs Dec/Jan prev-month columns), too brittle to gate on,
    # so the June file stays RED (vendor follow-up) rather than risk the baseline.
    # HMRS / ATLANTA — OPSTK/PURC/PSCH/IN/SALE/SSCH/OUT/STOCK scheme+transfer grid.
    if ("opstkpurcpschinsalesschoutstock" in flat and "stockandsalesstatement" in flat):
