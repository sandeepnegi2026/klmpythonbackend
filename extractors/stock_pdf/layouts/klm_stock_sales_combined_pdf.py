    ...
    with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for top, ws in lines[start:]:
                ...
                # 'Page N' footer: the word 'Page' sits far right (x0 ~ 467-473, well
                # past the pack column) with the page number beside it, so it has NO
                # name/pack tokens and would otherwise leak its page number into the last
                # product's closing column. Gate on the far-right x0 so a product name can
                # never be suppressed. Terminate the current block and drop the footer.
                if any(w["text"].lower() == "page" and w["x0"] > _PACK_MAX_X for w in ws):
                ...
                if has_rate:
                    ...
                    for w in num_toks:
                        ...
                        block["frags"].setdefault(field, []).append(
                            (top, w["x0"], w["text"]))
            # A product never wraps across a page break, so this page's last product
            # block is fully accumulated once its lines are consumed. Flush it at page
            # end; otherwise the next page's `block = new_block()` silently discards it
            # (the last real product of every non-final page was being dropped, E1).
            _flush(block, defs, records)
            block = new_block()
