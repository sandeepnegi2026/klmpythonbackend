...
WRAPPED ROWS (the tri-page pairing subtlety): when a product's name or strength cell
is too wide, Marg wraps it onto extra lines a few points above/below the row's
NUMERIC baseline — e.g. KERAMATE-HAIR-COLOUR-BLACK prints its name at top=285, its
code continuation '(KERAMK)' at top=294, while its numbers (and the matching page-B
row) sit at top=290. A naive exact-top join therefore (a) loses the whole page-B
side of such rows and (b) emits the continuation line ('(KERAMK) +60GM') as a
phantom product. Fix: page-A rows are first CLUSTERED into product bands (rows whose
name/strength/numeric content sits within _WRAP_GAP points of the previous such row
belong to the same product — real products are ~49pt apart, wrap sub-lines <=5pt),
the wrapped cell text is re-glued without a separator (Marg wraps mid-cell:
'SUSPENS'+'ION 60ML' -> 'SUSPENSION 60ML', '60GM+60GM'+'(KERAMK)' stays glued just
like the single-line rows print 'BOX(1000007029)'), and the page-B row is matched to
the cluster's NUMERIC baseline top (with a <=_WRAP_GAP tolerance fallback).
...
+ sales_return = closing. On this file ~118/162 rows balance exactly; the remainder
carry a printed PurchaseQty of 0.00 while their closing stock exceeds opening (the
vendor's Marg export prints 0.00 in BOTH PurchaseQty and PurchaseValue on every row
of every page even though stock grew — the vendor's own XLS twin of the same period
shows the same zeros) — a source imbalance, not a parse error. No number is
fabricated to force balance. The extracted sales_value / closing_stock_value sums
match the printed Grand Total row exactly, which value-corroborates the extraction.
...
# Wrapped sub-lines of ONE product row sit <=10pt from their neighbour sub-line
# (e.g. '60ML'@524 / 'SOLUTIO'@533 / 'N'@543); distinct product rows are ~49pt
# apart (worst inter-product sub-line gap ~29pt). 11pt cleanly separates the two
# regimes and is also the page-B row-matching tolerance (B rows are 49pt apart).
_WRAP_GAP = 11.0
...
def _glue(line_parts):
    """Re-join a wrapped NAME cell. The only wrapped name content is the glued
    product code ('...60GM+60GM'/'(KERAMK)'), which single-line rows print with no
    separator ('BOX(1000007029)'), so continuation lines are concatenated directly."""
    return "".join(p for p in (s.strip() for s in line_parts) if p)


def _glue_pack(line_parts, name):
    """Re-join a wrapped STRENGTH cell. Marg wraps it either mid-word
    ('SUSPENS'/'ION 60ML', 'SOLUTIO'/'N', 'CREAM-5'/'0GM' — re-glue directly) or at
    a token boundary ('60ML'/'SOLUTIO', '15GM'/'GEL' — keep the space). The product
    NAME repeats the full strength text, so the split kind is decided by whether the
    directly-glued boundary word occurs in the name; otherwise a space is kept."""
    parts = [p for p in (s.strip() for s in line_parts) if p]
    if not parts:
        return ""
    out = parts[0]
    for part in parts[1:]:
        prev_word = out.rsplit(None, 1)[-1]
        next_word = part.split(None, 1)[0]
        if prev_word + next_word in name:
            out += part
        else:
            out += " " + part
    return out
...
def parse_r15_nu_srishyam_sales_stock_detail_tripage(text, file_bytes=None):
    ...
    with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
        ...
        for base in range(0, n - 2, 3):
            ...
            b_tops = sorted(
                t for t, ws in rows_b.items()
                if any(_is_num(w["text"]) for w in ws)
            )

            # ---- cluster page-A rows into product bands (see WRAPPED ROWS above) ----
            # A row participates if it carries name text, strength text, or a numeric
            # token in one of the page-A numeric columns; the group-band column
            # (KLM/LABORATO/RIES/... at 340-392) never defines a band on its own.
            prod_tops = []
            for t in sorted(rows_a):
                ws = rows_a[t]
                has_name = any(
                    w["x0"] < _NAME_CUT and re.search(r"[A-Za-z]", w["text"])
                    for w in ws
                )
                has_pack = any(_NAME_CUT <= w["x0"] < _GROUP_LO for w in ws)
                has_num = any(
                    _is_num(w["text"]) and (
                        _near(w["x0"], _A_OPSTOCK)
                        or _near(w["x0"], _A_OPVALUE)
                        or _near(w["x0"], _A_PURQTY)
                    )
                    for w in ws
                )
                if has_name or has_pack or has_num:
                    prod_tops.append(t)

            clusters = []
            for t in prod_tops:
                if clusters and t - clusters[-1][-1] <= _WRAP_GAP:
                    clusters[-1].append(t)
                else:
                    clusters.append([t])

            for cl in clusters:
                line_ws = [sorted(rows_a[t], key=lambda w: w["x0"]) for t in cl]

                # product name/code = tokens left of the name cut, wrapped lines re-glued
                name = _glue(
                    " ".join(w["text"] for w in ws if w["x0"] < _NAME_CUT)
                    for ws in line_ws
                )
                ...
                # numeric page-A columns; remember the NUMERIC baseline top — the
                # page-B row of this product sits at that same top, not necessarily
                # at the (possibly wrapped) name line's top.
                op = pq = 0.0
                num_top = None
                for t, ws in zip(cl, line_ws):
                    for w in ws:
                        ...
                        if _near(w["x0"], _A_OPSTOCK):
                            ...
                            if num_top is None:
                                num_top = t
                        elif _near(w["x0"], _A_OPVALUE):
                            if num_top is None:
                                num_top = t
                ref = num_top if num_top is not None else cl[len(cl) // 2]
                ...
                # matching page-B row: same top as the numeric baseline (exact in
                # practice; <=_WRAP_GAP tolerance keeps a 1-2pt rounding drift safe)
                sq = sv = cs = cv = prq = srq = 0.0
                bt = min(b_tops, key=lambda t: abs(t - ref), default=None)
                brow = rows_b.get(bt, []) if bt is not None and abs(bt - ref) <= _WRAP_GAP else []
                for w in brow:
                    if not _is_num(w["text"]):
                        continue
                    x = w["x0"]
