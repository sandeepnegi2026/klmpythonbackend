...
def parse_busy_tally(text):
    ...
    # SALE-RETURN rows with a POSITIVE-INTEGER qty and NEGATIVE money columns
    # ("EZACNE SACHET 1*1 10 -24 -32.54 -325.35 -0.04"): a return prints a normal
    # integer Qty but signed (negative) Free/Rate/Amount/(%). pat6/pat5 reject the
    # leading '-' in their rate/amount classes ([\d.]+), and pat6_frac/pat5_frac
    # require the QTY itself to be decimal-or-negative, so this exact shape
    # (int qty + signed-money tail) fell through EVERY branch and the row was
    # silently dropped (its -Amount then made the extracted sum overshoot the
    # printed GRAND TOTAL by |Amount|). Tried ONLY after every pattern above
    # fails, and accepted ONLY when a negative money value is actually present, so
    # no all-positive row (already taken by pat6/pat5) can ever reach or match it.
    _free_sgn = r"(-|-?\d+(?:\.\d{1,3})?)"
    pat6_neg = re.compile(
        r"^(.+?)\s+(\d+)\s+" + _free_sgn + r"\s+" + _money + r"\s+" + _money + r"\s+" + _money + r"\s*$"
    )
    pat5_neg = re.compile(
        r"^(.+?)\s+(\d+)\s+" + _free_sgn + r"\s+" + _money + r"\s+" + _money + r"\s*$"
    )
    ...
    for line in text.split("\n"):
        ...
        if mc:
            ...
            continue
        # NEGATIVE-money return fallback (see pattern note). Emit only when a
        # negative money value is present, so an all-positive row can never divert
        # here from pat6/pat5. Does NOT flip has_int_row (mirrors the frac/colon
        # rescues): a file whose ONLY rows are these must still defer to its
        # dedicated parser rather than be hijacked by a partial busy_tally parse.
        mn = pat6_neg.match(s) if has_pct else pat5_neg.match(s)
        if mn and "-" in (mn.group(4) + mn.group(5)):
            name, area = _split_party_area(cur_raw)
            free = mn.group(3)
            rows.append(
                [name, area, mn.group(1).strip(), mn.group(2),
                 "0" if free == "-" else free, mn.group(4), mn.group(5), vendor_line]
            )
            continue
