...
from core.party_filter import filter_generic_accounts
...
def extract(file_bytes, settings=None):
    ...
    # Drop generic non-customer ledger accounts (CASH / COUNTER / WALK IN / STAFF / CASH MEMO
    # / CASH COLLECTION <city> / ...). Runs AFTER the line ledger (computed in pdf_io on the raw
    # parser output), so those lines stay CLAIMED — intentionally excluded, never counted as
    # dropped. Name-anchored allowlist (core/party_filter): a real shop that merely contains a
    # keyword ("CASH CHEMIST", "GENERAL MEDICAL STORES") is untouched, so every file without such
    # an account is byte-for-byte unchanged.
    canonical_rows, excluded_generic = filter_generic_accounts(canonical_rows)
    ...
    return {
        ...
        "debug": {
            ...
            "excluded_generic_party_rows": excluded_generic,
        },
    }
