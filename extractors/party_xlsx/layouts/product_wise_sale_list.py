...
def parse_product_wise_sale_list(rows):
    ...
    for i, row in enumerate(body):
        ...
        if nxt is not None and _c(nxt, prod_col) == part and _is_num(_c(nxt, qty_col)):
            continue
        # This layout's party is a hyphenated BRAND ("DEV-PUSHP MEDICAL STORE RAJKOT",
        # "ZYRA-PHARMACY IRITTY"), NOT a "NAME-CITY" pair, so never hyphen-split.
        pname, ploc = split_party_area(part, band_brand=True)
        party = pname or part
