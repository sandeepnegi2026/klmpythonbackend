...
def _header_spacer_idxs(rows, header_idx, headers, col):
    """Interior EMPTY header cells that sit between two mapped data columns.

    A family of KLM "Company-Party wise Sale" .xls exports prints the header with a
    blank spacer cell (``Pak | <blank> | Qty | Schm | Rate | Amount``) that the data
    rows below DO NOT carry — the product rows are one cell shorter. Because the column
    map is built from the header's own indices, every field to the RIGHT of the spacer
    reads one cell too far on the data rows (``qty`` grabs Schm, ``amount`` runs off the
    end -> qty/amount all 0, ``rate`` holds the printed Amount). This returns the sorted
    indices of those blank interior header cells so the reader can compensate.

    Gated hard: only cells that (a) are blank in the header, (b) fall strictly between the
    smallest and largest MAPPED column index (a genuine interior spacer, not trailing
    padding), and (c) are not themselves a mapped column. Files whose header carries no
    such interior blank (e.g. merged "Customer-Company wise…" bands) return ``[]`` and are
    completely untouched.
    """
    # Only REAL (non-raw_) mapped columns define the data region; a blank header cell that
    # picked up a synthetic ``raw_col_N`` name is exactly the spacer we want to catch.
    real = [i for k, i in col.items() if not k.startswith("raw_") and isinstance(i, int)]
    if len(real) < 2:
        return []
    ...
    for i in range(lo + 1, hi):
        if i in real_set:
            continue
        if i < len(hdr) and not cell_text(hdr[i]):
            out.append(i)
    return out


def _shift_for_row(idx, raw_len, header_len, spacers):
    """Map a header-based column index onto a data row that OMITS the header spacer cells.

    When the data row is shorter than the header by exactly the number of interior spacer
    cells that precede ``idx``, subtract that count so the field lands on the right cell.
    Only fires when the row is short by the expected amount; full-width rows (and the header
    row itself) are returned unchanged, so nothing is disturbed on well-formed data."""
    if not spacers or raw_len >= header_len:
        return idx
    preceding = sum(1 for s in spacers if s < idx)
    # only compensate when the row is short by the spacers before this column
    if preceding and raw_len == header_len - len([s for s in spacers if s < header_len]):
        return idx - preceding
    return idx
...
def parse_customer_product_banded(rows):
    ...
    # Interior blank header spacer(s) that the data rows below omit (KLM Company-Party wise
    # Sale .xls). When present, per-row column indices to their right are shifted left so
    # qty/amount/rate/free land on the right cell instead of reading one cell too far.
    header_len = len(rows[header_idx])
    spacers = _header_spacer_idxs(rows, header_idx, headers, col)
    ...
    for raw in rows[header_idx + 1 :]:
        ...
        for key, idx in col.items():
            eff = _shift_for_row(idx, len(raw), header_len, spacers)
            record[key] = raw[eff] if eff < len(raw) else ""
