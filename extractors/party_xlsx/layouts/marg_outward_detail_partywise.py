...
# The Shresth "GRP" variant of this export truncates its header text (Produ | Qty | InvNo |
# InvDate | BatchNo | ExpDate | MRP | Ledger Account | Area | CCode). "Produ" then binds
# hsn_code (substring of the "product code" synonym), product_name never maps, detect()
# rejects the file and it falls to generic tabular with every product name lost. Gate on the
# exact compact run of ALL 10 truncated columns — corpus-unique, so no other variant of this
# family (SATARA GrsAmt-style, SIDDHARTH NetAmt-style) can enter the rename branch.
_TRUNC_HEADER_RUN = "produqtyinvnoinvdatebatchnoexpdatemrpledgeraccountareaccode"
...
def detect(rows):
    ...
    if header_idx is None:
        return False
    # Truncated GRP header run: product_name cannot map until parse() renames "Produ",
    # so the mapped-keys check below would wrongly reject the file.
    if _TRUNC_HEADER_RUN in head:
        return True
    keys = {info["canonical"]
...
def parse_marg_outward_detail_partywise(rows):
    ...
    # Truncated GRP run only (see _TRUNC_HEADER_RUN): give the two ambiguous truncated
    # headers unambiguous names before mapping. "Produ" -> exact product_name match;
    # "CCode" (customer code) would otherwise bind hsn_code once "Produ" stops taking it.
    if _TRUNC_HEADER_RUN in compact(" ".join(headers)):
        headers = [
            "Product Name" if normalize(h) == "produ"
            else "Cust Ref Raw" if normalize(h) == "ccode"
            else h
            for h in headers
        ]
