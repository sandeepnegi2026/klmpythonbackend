...
def extract(file_bytes, settings=None):
    ...
    # Drop generic non-customer ledger accounts (CASH / COUNTER / WALK IN / STAFF / ...) AFTER
    # the ledger runs on the full record set, so those rows stay CLAIMED (intentionally excluded,
    # not dropped). Name-anchored allowlist (core/party_filter) never removes a real shop.
    records, _excluded_generic = filter_generic_accounts(records)
    ...
    return {
        ...
        "debug": {
            ...
            "sheet": sheet_name,
            "excluded_generic_party_rows": _excluded_generic,
        },
    }
...
def _extract_html(file_bytes, started):
    ...
    records, _ = filter_generic_accounts(records)
