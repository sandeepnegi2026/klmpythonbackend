...
PARTY_FIELDS = {
    ...
    # A "Sale Rate"/"S Rate"/"Selling Rate" column is the per-line selling rate — the
    # same thing our `rate` field carries (the PDF layouts hard-map it to `rate`). It has
    # no exact synonym here, so header_match binds it to `rate` via the 0.88 "rate"⊂"sale
    # rate" contains-rule; a real "Rate"/"Price" column (exact 1.0) always outranks it, so
    # a sheet with BOTH keeps its real rate and the sale-rate column falls to raw_ (PRASU).
    # "purc rate" — SwilERP/Marg abbreviation for the purchase rate (PRASU PHARMA tabular
    # export has BOTH "Purc rate" and "Sale rate"). Without the EXACT hit, "purc rate"
    # contains-matches `rate` (0.88) and STEALS it from the real "Sale rate", leaving the
    # selling rate in raw_. The exact 1.0 here binds it to purchase_rate, freeing `rate`
    # for "Sale rate" (0.88 contains). Verified single-column "Purc rate" files don't exist
    # in-corpus, so `rate` never goes unmapped.
    "purchase_rate": {"scope": "line_item", "type": "num", "required": False, "synonyms": ["purchase rate", "pur rate", "p rate", "purc rate"]},
