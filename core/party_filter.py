...
def tag_generic_accounts(rows):
    """Mark generic non-customer ledger accounts (CASH / COUNTER / WALK IN / STAFF /
    CASH MEMO / CASH COLLECTION <city> / ...) with ``is_generic_party = True`` instead
    of DROPPING them, so every row is retained (nothing lost) while party-wise reads
    can still exclude them at query time. Same anchored allowlist as
    filter_generic_accounts, so a real shop that merely contains a keyword
    ("CASH CHEMIST", "GENERAL MEDICAL STORES") is never tagged.

    Real-party rows are left byte-for-byte untouched — the key is added ONLY to the
    generic rows — so any file without a generic account produces identical output.
    Returns (rows, n_tagged)."""
    tagged = 0
    for r in rows:
        if is_generic_nonparty_account(r.get("party_name")):
            r["is_generic_party"] = True
            tagged += 1
    return rows, tagged
